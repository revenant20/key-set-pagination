...
@Slf4j
public abstract class KeySetByShortFilterWalker<T extends Ent> extends BaseWalker {

    private final DataRepository<T, String> repository;

    public KeySetByShortFilterWalker(DataRepository<T, String> repository) {
    ...
    protected void walk(UploadCommand command, SimpleCsvWriter<PageLoadDuration> writer) {
        ...
        for (var i = 0; ; i++) {
            var start = now();
            var rows = repository.findAllByShortFilter(size, lastDate, lastId);
            var end = now();
