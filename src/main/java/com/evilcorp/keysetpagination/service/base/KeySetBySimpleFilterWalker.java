...
@Slf4j
public abstract class KeySetBySimpleFilterWalker<T extends Ent> extends BaseWalker {

    private final DataRepository<T, String> repository;

    public KeySetBySimpleFilterWalker(DataRepository<T, String> repository) {
    ...
    protected void walk(UploadCommand command, SimpleCsvWriter<PageLoadDuration> writer) {
        ...
        for (var i = 1; ; i++) {
            start = now();
            var rows = repository.findAllBySimpleFilter(Pageable.ofSize(size), lastDate, lastId);
            end = now();
