...
@Slf4j
public abstract class KeySetBySimpleFilterWalkerWithReverseOrder<T extends Ent> extends BaseWalker {

    private final DataRepository<T, String> repository;

    public KeySetBySimpleFilterWalkerWithReverseOrder(DataRepository<T, String> repository) {
    ...
    protected void walk(UploadCommand command, SimpleCsvWriter<PageLoadDuration> writer) {
        ...
        for (var i = 1; ; i++) {
            start = now();
            var rows = repository.findAllBySimpleFilterWithReverseOrder(Pageable.ofSize(size), lastDate, lastId);
            end = now();
